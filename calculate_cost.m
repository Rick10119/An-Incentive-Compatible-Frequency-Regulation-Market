...
allocate.cost_deg = zeros(TIME, 1);      % Degradation cost for primary storage
allocate.cost_deg2 = zeros(TIME, 1);     % Degradation cost for secondary storage
allocate.cost_rp = zeros(TIME, NOFTYPES + 2);  % Resource-specific costs
...
for t_cap = 1 : signal_length
    ...
    % Calculate degradation cost for primary storage system
    ...
    % Calculate degradation cost for secondary storage system
...
allocate.cost_rp(:, 4) = allocate.cost_rp(:, 4) + allocate.cost_rp(:, 5);  % Combine primary storage
allocate.cost_rp(:, 6) = allocate.cost_rp(:, 6) + allocate.cost_rp(:, 7);  % Combine secondary storage
allocate.cost_rp = [allocate.cost_rp(:, 1), allocate.cost_rp(:, 3:4), allocate.cost_rp(:, 6)];  % Final resource grouping
...
allocate.cost_es = sum(allocate.cost_rp(:, 3) + allocate.cap_cost(:, 4));                 % Primary energy storage
allocate.cost_es2 = sum(allocate.cost_rp(:, 4) + allocate.cap_cost(:, 5));                % Secondary energy storage
